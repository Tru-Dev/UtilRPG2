defmodule UtilRPG2.Commands.Dice.D12 do
  ...
  def get, do: %{
    name: "d12",
    description: "Rolls one or more d12s.",
    options: [
      %{
        type: 4,
        name: "amount",
        description: "Amount of d12s to roll (default 1).",
        min_value: 1,
      },
      %{
        type: 4,
  ...
  def run(%Interaction{data: %{options: [%{name: "amount", value: amt}, %{name: "modifier", value: mod}]}}) do
    %{
      content: Util.gen_roll_result(amt, 12, mod)
    }
  end

  def run(%Interaction{data: %{options: [%{name: "amount", value: amt}]}}) do
    %{
      content: Util.gen_roll_result(amt, 12, 0)
    }
  end

  def run(%Interaction{data: %{options: [%{name: "modifier", value: mod}]}}) do
    %{
      content: Util.gen_roll_result(1, 12, mod)
    }
  end

  def run(%Interaction{}) do
    %{
      content: Util.gen_roll_result(1, 12, 0)
    }
  end
end
